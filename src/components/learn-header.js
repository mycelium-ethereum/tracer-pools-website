/* eslint-disable */
import React from "react";

const LearnHeader = () => {
  return (
    <>
      <header className="h-96 flex md:items-end md:pt-0 pt-20 items-center z-20 relative">
        <div className="container w-full mx-auto flex flex-col justify-center text-center pt-24 lg:px-0 px-4">
          <small className="md:text-3xl text-xl color-blue font-bold">
            Learn
          </small>
          <h1 className="font-black md:text-6xl text-4xl text-white mb-2">
            Tracer Research
          </h1>
          <span className="md:text-2xl text-lg text-white font-normal">
            Gain a more in-depth understanding of the Tracer protocol
          </span>
        </div>
      </header>
    </>
  );
};

export default LearnHeader;
