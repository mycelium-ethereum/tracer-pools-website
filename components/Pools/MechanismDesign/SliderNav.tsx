...
const SliderNav: React.FC<{
  ...
  const SlideNavLink: React.FC<{ num: number }> = ({ num }) => (
    <button
      className="relative mx-6 text-2xl font-light leading-[30px] text-action-inactive transition-colors duration-300 sm:mx-0 sm:my-[15px] 3xl:my-[22px] 3xl:text-[40px] 3xl:leading-[44px]"
      onClick={() => pauseSlider()}
    >
      {num}
    </button>
  ...
  return (
    <div className="relative mx-auto mt-4 min-w-[43px] px-6 sm:mr-10 sm:ml-[23px] sm:mt-0 sm:px-0 3xl:ml-4 3xl:mr-16 3xl:min-w-[42px]">
      <Slider
        asNavFor={slider2}
        ref={(slider) => setSlider1(slider)}
        {...sliderNavSettings}
        className="slider-nav relative z-10 sm:h-[440px] sm:w-[43px] 3xl:h-[635px] 3xl:w-[42px]"
      >
